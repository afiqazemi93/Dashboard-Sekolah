...
const DEFAULT_PPKI_GAS_URL =
  "https://script.google.com/macros/s/AKfycbxREseFnc1-HopWKZ9Oa5ogwljzR7M3iqYBvWo71GNx_nZDQON-uCuI5W-WSMd7j635Ug/exec";
const DEFAULT_PEMULIHAN_GAS_URL =
  "https://script.google.com/macros/s/AKfycbzE8Jh3iCLiecNbv7HD9d3Qg6byWdR_aGVXamZLJDsoMWbWLEo2coOmL7jYuKV7_6AChA/exec";
...
export function PpkiPemulihanView({
  details,
  isAdmin,
  onSave,
}: PpkiPemulihanViewProps) {
  ...
  const [formPpkiStaff, setFormPpkiStaff] = useState<string[]>(
    details.ppkiStaffIds || [],
  );
  const [formPemulihanStaff, setFormPemulihanStaff] = useState<string[]>(
    details.pemulihanStaffIds || [],
  );
    ...
    try {
      ...
      try {
        ...
        if (text.trim().startsWith("<")) {
          throw new Error(
            "Apps Script memulangkan HTML. Pastikan Apps Script anda menggunakan ContentService untuk memulangkan format JSON, bukan HtmlService.",
          );
        }
      ...
      if (err.message === "Failed to fetch") {
        setErrorMsg(
          `Gagal mengambil data ${type.toUpperCase()}: Ralat CORS atau URL App Script disekat. Pastikan App Script di'deploy' dengan tetapan: "Execute as: Me" dan "Who has access: Anyone". Jika menggunakan akaun MOE (moe-dl.edu.my), akses mungkin disekat oleh domain.`,
        );
      } else {
        setErrorMsg(
          `Gagal mengambil data ${type.toUpperCase()}: ${err.message}`,
        );
      }
    } finally {
      setLoading(false);
  ...
  const getAnalytics = (data: any[], type: "ppki" | "pemulihan") => {
    ...
    const getKey = (item: any, possibleKeys: string[]) => {
      for (const key of possibleKeys) {
        const matchingKey = Object.keys(item).find((k) =>
    ...
    data.forEach((item) => {
      ...
      if (jantina && typeof jantina === "string") {
        if (
          jantina.toUpperCase().includes("LELAKI") ||
          jantina.toUpperCase() === "L"
        )
          lelaki++;
        else perempuan++;
      }
      ...
      if (type === "ppki") {
        ...
        if (ketidakupayaanRaw) {
          ...
          const properCaseKetidakupayaan =
            ketidakupayaan.charAt(0) + ketidakupayaan.slice(1).toLowerCase();
          disabilityCount[properCaseKetidakupayaan] =
            (disabilityCount[properCaseKetidakupayaan] || 0) + 1;
        }
    ...
    const categoryChart = Object.keys(categoryCount).map((k) => ({
      name: k,
      value: categoryCount[k],
    }));
    const disabilityChart = Object.keys(disabilityCount).map((k) => ({
      name: k,
      value: disabilityCount[k],
    }));
    const bangsaChart = Object.keys(bangsaCount).map((k) => ({
      name: k,
      value: bangsaCount[k],
    }));
    const programChart = Object.keys(programCount).map((k) => ({
      name: k,
      value: programCount[k],
    }));
    const tahunChart = Object.keys(tahunCount).map((k) => ({
      name: k,
      value: tahunCount[k],
    }));
  ...
  const renderTableHeader = () => {
    ...
    const keys = Object.keys(currentData[0]).filter(
      (k) =>
        !k.toUpperCase().includes("MYKID") &&
        !k.toUpperCase().includes("TARIKH LAHIR") &&
  ...
  const renderTableBody = () => {
    ...
    if (activeTab === "pemulihan" && currentData.length > 0) {
      fallbackKeys = Object.keys(currentData[0]).filter(
        (k) =>
          !k.toUpperCase().includes("MYKID") &&
          !k.toUpperCase().includes("TARIKH LAHIR") &&
          !k.toUpperCase().includes("ID") &&
      ...
      const nameIndex = fallbackKeys.findIndex((k) =>
        k.toUpperCase().includes("NAMA"),
      );
      if (nameIndex > 0) {
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in duration-300">
              ...
              <form onSubmit={handleSaveSettings} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <input
                      type="text"
                      value={formPpkiUrl}
                      onChange={(e) => setFormPpkiUrl(e.target.value)}
                      required
                      className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-bold focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all"
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    URL App Script Pemulihan
                  </label>
                  <div className="flex gap-2">
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center px-4">
                      <Link2 className="w-4 h-4 text-slate-400" />
                    ...
                    <input
                      type="text"
                      value={formPemulihanUrl}
                      onChange={(e) => setFormPemulihanUrl(e.target.value)}
                      required
                      className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-bold focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all"
                    />
                  </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 max-h-[200px] overflow-y-auto space-y-2">
                      {allStaff.map((s) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={formPpkiStaff.includes(s.id)}
                            onChange={(e) => {
                              if (e.target.checked)
                                setFormPpkiStaff([...formPpkiStaff, s.id]);
                              else
                                setFormPpkiStaff(
                                  formPpkiStaff.filter((id) => id !== s.id),
                                );
                            }}
                      ...
                      {allStaff.length === 0 && (
                        <span className="text-xs text-slate-400">
                          Tiada rekod guru/staf. Sila tambah di tab Pentadbiran
                          & Staf.
                        </span>
                      )}
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 max-h-[200px] overflow-y-auto space-y-2">
                      {allStaff.map((s) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={formPemulihanStaff.includes(s.id)}
                            onChange={(e) => {
                              if (e.target.checked)
                                setFormPemulihanStaff([
                                  ...formPemulihanStaff,
                                  s.id,
                                ]);
                              else
                                setFormPemulihanStaff(
                                  formPemulihanStaff.filter(
                                    (id) => id !== s.id,
                                  ),
                                );
                            }}
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                  <button
                    ...
                    className="px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-400 hover:bg-slate-100 transition-colors"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    className="px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-200 transition-all"
                  >
                    Simpan Tetapan
                  </button>
                </div>
      ...
      {(() => {
        ...
        const assignedStaff = currentStaffIds
          .map((id) => allStaff.find((s) => s.id === id))
          .filter(Boolean);
        if (assignedStaff.length === 0) return null;

        const title =
          activeTab === "ppki" ? "Guru & Staf (PPKI)" : "Guru Pemulihan";

        return (
          <div
            className={`filter drop-shadow-sm p-6 rounded-3xl mx-auto flex flex-col items-center justify-center mb-8 max-w-fit border bg-white ${activeTab === "ppki" ? "border-blue-100" : "border-emerald-100"}`}
          >
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {assignedStaff.map((s) => (
                <div
                  key={s!.id}
                  className="bg-white rounded-[16px] p-3 flex items-center space-x-4 w-full sm:w-[260px] shadow-[0_4px_15px_rgba(0,0,0,0.05)] hover:shadow-[0_4px_20px_rgba(0,0,0,0.1)] transition-all outline-none border-none border-transparent"
                >
                  <div className="relative w-14 h-14 shrink-0 select-none">
                    <img
                      ...
                      src={
                        s!.photoUrl ||
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(s!.name)}&background=EBF4FF&color=3B82F6`
                      }
                      alt={s!.name}
                    ...
                    {s!.grade && (
                      <span
                        className={`absolute -bottom-2 left-1/2 -translate-x-1/2 px-2.5 py-1 text-[10px] font-black text-white rounded-full leading-none border-2 border-white shadow-md z-10 whitespace-nowrap ${activeTab === "ppki" ? "bg-blue-600" : "bg-emerald-600"}`}
                      >
                        {s!.grade}
                      </span>
                    )}
                  ...
                  <div className="min-w-0 flex-1 text-left">
                    {s!.subject && (
                      <p
                        className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-widest border px-2 py-0.5 rounded-full inline-block mb-1.5 ${activeTab === "ppki" ? "bg-blue-50 text-blue-700 border-blue-100" : "bg-emerald-50 text-emerald-700 border-emerald-100"}`}
                      >
                        {s!.subject}
                      </p>
                    )}
                    <h5
                      className="font-extrabold text-[12px] sm:text-[13px] text-gray-950 line-clamp-2 leading-snug mb-1"
                      title={s!.name}
                    >
                      {s!.name}
                    </h5>
      ...
      {!loading && !errorMsg && currentData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div
            className={`p-8 rounded-3xl border shadow-sm relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 ${activeTab === "ppki" ? "bg-blue-600 border-blue-500 lg:col-span-1" : "bg-emerald-600 border-emerald-500 lg:col-span-1"}`}
          >
            <div className="flex flex-col items-center md:items-start z-10">
              <p className="text-xs font-black uppercase tracking-widest mb-2 text-white/80">
                Jumlah Murid
              </p>
            ...
            <div className="flex gap-8 items-center bg-black/10 px-8 py-6 rounded-3xl backdrop-blur-sm z-10 w-full md:w-auto justify-center">
              <div className="text-center">
                ...
                <p className="text-[10px] font-black text-white/80 uppercase tracking-widest mt-2 flex items-center gap-1">
          ...
          {activeTab === "ppki" && analytics.bangsaChart.length > 0 && (
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col h-full lg:col-span-1">
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 w-full mb-6">
                Pecahan Bangsa
              </h3>
              <div className="flex gap-6 items-center flex-1">
                <div className="h-[120px] w-[120px] shrink-0 relative">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Tooltip
                        formatter={(value) => [`${value} Murid`, "Jumlah"]}
                        contentStyle={{
                          borderRadius: "16px",
                          border: "none",
                      ...
                      />
                      <Pie
                        data={analytics.bangsaChart}
                        dataKey="value"
                        innerRadius={35}
                        outerRadius={60}
                        paddingAngle={2}
                      >
                        {analytics.bangsaChart.map((e, index) => (
                          <Cell
                            key={`cell-${index}`}
                  ...
                  {analytics.bangsaChart.map((c, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2.5 flex-1 min-w-0">
                        ...
                        ></div>
                        <span className="font-bold text-slate-600 uppercase truncate leading-tight">
                          {c.name}
                        </span>
                      </div>
          ...
          {activeTab === "pemulihan" &&
            analytics.programChart &&
            analytics.programChart.length > 0 && (
                    ...
                    .map((c, i) => (
                      <div key={i} className="flex flex-col gap-2">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md">
                            {c.value}
                          </span>
                        </div>
                        <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
      ...
      {activeTab === "pemulihan" &&
        analytics.tahunChart &&
        analytics.tahunChart.length > 0 && (
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col h-full overflow-hidden col-span-full mt-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 w-full mb-6 font-sans">
              Jumlah Murid Pemulihan Mengikut Tahun
            </h3>
                ...
                {analytics.tahunChart
                  ...
                  .map((c, i) => {
                    const maxVal = Math.max(
                      ...analytics.tahunChart.map((y) => y.value),
                      1,
                    );
                    ...
                    return (
                      <div
                        key={i}
                        className="flex-1 flex flex-col items-center group relative h-full justify-end z-10 min-w-0"
                      >
                        {/* Hover Tooltip/Value */}
                        <div className="absolute bottom-full mb-2 bg-slate-900 text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-250 pointer-events-none whitespace-nowrap">
                          {c.value} Murid (
                          {Math.round((c.value / (analytics.total || 1)) * 100)}
                          %)
                        </div>

                        {/* Static Value Counter above the bar */}
                        <span className="text-[10px] sm:text-xs font-extrabold mb-1.5 text-emerald-700">
                          {c.value}
                        </span>
                        ...
                        {/* Year Label */}
                        <span
                          className="text-[8px] sm:text-[10px] font-black text-slate-500 uppercase tracking-tight mt-2 text-center truncate w-full"
                          title={c.name}
                        >
                          {c.name}
                        </span>
        ...
        <div className="p-4 sm:p-6 border-b border-slate-100 bg-white flex flex-col sm:flex-row gap-4 justify-between sm:items-center">
          <div>
            <h3 className="font-black text-slate-800 tracking-tight flex items-center gap-2">
              <UsersRound
                className={`w-5 h-5 ${activeTab === "ppki" ? "text-blue-500" : "text-emerald-500"}`}
              />
              {activeTab === "ppki"
                ? "Senarai Murid PPKI"
                : "Senarai Murid Pemulihan"}
            </h3>
          </div>
          ...
          <div className="flex flex-1 sm:flex-none items-center gap-3 w-full sm:w-auto">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 text-slate-500 ${loading ? "animate-spin" : "group-hover:text-blue-600"}`}
              />
            </button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto w-full min-h-[300px] relative">
          {loading && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
              ...
              <p className="mt-4 text-sm font-black text-slate-600 uppercase tracking-widest animate-pulse">
              ...
              <button
                ...
                className={`p-2 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-100 transition-colors disabled:opacity-40 disabled:hover:bg-transparent ${
                  currentPage === 1
                    ...
                    : "cursor-pointer px-3 bg-white shadow-sm"
                }`}
              >
