...
export function KurikulumView({
  ...
  const [selectedPanitiaForOrg, setSelectedPanitiaForOrg] =
    useState<Panitia | null>(null);
  const [isTeacherSelectModalOpen, setIsTeacherSelectModalOpen] =
    useState(false);
  const [teacherSearchQuery, setTeacherSearchQuery] = useState("");
  const [memberRoleToAssign, setMemberRoleToAssign] =
    useState<PanitiaMember["role"]>("Guru Panitia");
  ...
  const handleSavePanitia = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingPanitia) {
      newList = newList.map((p) =>
        p.id === editingPanitia.id ? { ...p, name, color, slug } : p,
      );
    } else {
      newList.push({
        id: Math.random().toString(36).substring(2, 9),
  ...
  const handleBannerUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      const url = await uploadBase64ToStorage(
        `kurikulum/uasa_banner_${Date.now()}.webp`,
        base64,
      );
      onSave({
        ...details,
  ...
  const handleScreenshotUpload = async (
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const url = await uploadRawFileToStorage(
          `kurikulum/uasa_screenshot_${Date.now()}_${i}.${file.type.split("/")[1] || "jpg"}`,
          file,
        );
        newUrls.push(url);
      }
      ...
      if (errMsg.toLowerCase().includes("security policy")) {
        setUploadErrorMsg(
          "Ralat Sekuriti (RLS). Sila salin skrip SQL di panel utama sistem untuk cipta & benarkan akses kepada Bucket 'school_media' dalam Supabase anda.",
        );
      } else {
          ...
          {filteredPanitia.map((p) => (
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                {(
                  p.members.filter(
                    (m) => m.role === "Ketua Panitia",
                  ) as PanitiaMember[]
                ).map((m) => (
                  <div
                    key={m.id}
                    className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center gap-3"
                  >
      ...
      {filteredPanitia.length === 0 && (
        ...
        >
          ...
          <h3 className="text-xl font-bold text-slate-700 mb-2">
            Tiada Panitia Dijumpai
          </h3>
          <p className="text-slate-400 max-w-xs text-sm">
            Sila cuba carian lain atau tambah panitia baharu.
          </p>
        </motion.div>
      )}
    </div>
  ...
  const renderPpkiTab = () => (
    <div className="flex flex-col items-center justify-center py-32 text-center bg-white/50 border border-white/20 backdrop-blur-xl rounded-[3rem] shadow-xl shadow-slate-200/20">
      ...
      <h3 className="text-3xl font-black text-slate-800 mb-3 tracking-tight">
        Data Murid PPKI & Pemulihan
      </h3>
      <p className="text-slate-500 max-w-md mx-auto leading-relaxed px-6">
        Bahagian ini akan dikemaskini dengan maklumat terperinci mengenai
        program pendidikan khas integrasi dan kelas pemulihan khas.
      </p>
    </div>
  );
          ...
          <div className="p-10 bg-slate-50/50 flex flex-col items-center gap-6 border-t border-slate-100">
            <div className="text-center max-w-2xl">
              <h4 className="text-xl font-black text-slate-800 mb-2 tracking-tight uppercase">
                Sistem Integrasi Markah & PBD (SIMP)
                <br />
                <span className="text-blue-600">SK Batu Lanchang</span>
              </h4>
              <p className="text-sm text-slate-500 leading-relaxed">
                Platform digital pintar yang mengintegrasikan pengurusan markah,
                Tahap Penguasaan (TP) dan analitik prestasi murid dalam satu
                sistem bersepadu. Sila klik butang di bawah untuk akses terus ke
                portal.
              </p>
            </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onSave({
                        ...
                        kurikulumData: {
                          ...kurikulumData,
                          simpButtonLabel: e.target.value,
                        },
                      })
                    }
                  ...
                  {uploadErrorMsg.includes("Sekuriti") && (
                    <div className="mt-2 text-xs bg-white border border-red-100 p-4 rounded-xl flex flex-col gap-2">
                      <p className="font-semibold text-slate-700">
                        Skrip Penyelesaian (Salin & laksanakan di SQL Editor
                        Supabase anda):
                      </p>
...
create policy "Public Delete" on storage.objects for delete to public using ( bucket_id = 'school_media' );`}
          ...
          <div className="p-8 sm:p-10 bg-slate-50/50">
            {!kurikulumData.uasaPbdScreenshots ||
            kurikulumData.uasaPbdScreenshots.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                {isUploadingScreenshot ? (
                  ...
                  <>
                    ...
                    <p className="font-black text-sm uppercase tracking-widest">
                      Tiada tangkap layar aplikasi tersedia
                    </p>
                  </>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                {kurikulumData.uasaPbdScreenshots.map((url, idx) => (
                  <div
                    key={url + idx}
                    className="relative group w-full aspect-[4/3] sm:aspect-video rounded-3xl overflow-hidden shadow-md border border-slate-200 bg-white"
                  >
                    <img
                      src={url}
                      ...
                      <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="flex flex-col gap-1 bg-white/90 backdrop-blur-sm p-1 rounded-xl shadow-lg border border-slate-200">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleReorderScreenshot(idx, "up");
                            }}
                            disabled={idx === 0}
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleReorderScreenshot(idx, "down");
                            }}
                            disabled={
                              idx ===
                              kurikulumData.uasaPbdScreenshots!.length - 1
                            }
                            className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 disabled:opacity-30 disabled:hover:bg-transparent rounded-lg transition-colors"
                          >
                ...
                {isUploadingScreenshot && (
                  <div className="w-full aspect-[4/3] sm:aspect-video rounded-3xl overflow-hidden shadow-md border border-slate-200 bg-white flex flex-col items-center justify-center animate-pulse">
                    ...
                    <p className="text-sm font-bold text-slate-500 uppercase tracking-widest text-center px-4">
                      Memuat Naik...
                    </p>
                  </div>
                )}
    ...
    <div className="pb-20 w-full">
      <div className="flex flex-col sm:flex-row gap-4 justify-between sm:items-center bg-white p-6 sm:p-8 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] mb-8">
        <div className="flex items-center space-x-4">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center border shadow-sm ${headerInfo.colorClasses}`}
          >
          ...
          >
            <div className="bg-slate-50 px-10 py-8 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-2xl font-black text-slate-800 tracking-tight">
                {editingPanitia ? "Edit Panitia" : "Tambah Panitia"}
              </h3>
              <button
                onClick={() => {
                  setIsPanitiaModalOpen(false);
                  ...
                  {[
                    ...
                    >
                      <input
                        ...
                        name="color"
                        value={color}
                        defaultChecked={editingPanitia?.color === color}
            ...
            <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between bg-white relative">
              <div className="flex items-center gap-4">
                <div
                  ...
                  style={{ backgroundColor: selectedPanitiaForOrg.color }}
                >
                  <BookOpen className="w-6 h-6 text-white" />
                </div>
                <div>
                  <h3 className="text-2xl font-black text-slate-800 tracking-tight leading-none uppercase">
                    {selectedPanitiaForOrg.name}
                  </h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                    JUMLAH AHLI: {selectedPanitiaForOrg.members.length}
                  </p>
                </div>
              </div>
              <button
              ...
              <section>
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="w-full flex justify-center">
                    {(
                      selectedPanitiaForOrg.members.filter(
                        (m) => m.role === "Ketua Panitia",
                      ) as PanitiaMember[]
                    ).length > 0 ? (
                      (
                        ...
                        <OrgCard
                          key={m.id}
                          member={m}
                          panitiaColor={selectedPanitiaForOrg.color}
                          onRemove={() => {
                            const newMembers =
                              selectedPanitiaForOrg.members.filter(
                                (mem) => mem.id !== m.id,
                            ...
                            handleUpdateOrg(
                              selectedPanitiaForOrg.id,
                              newMembers,
                            );
                            setSelectedPanitiaForOrg({
                              ...selectedPanitiaForOrg,
                              members: newMembers,
                            });
                          }}
                          isAdmin={isAdmin}
                      ...
                      <div className="p-6 border-2 border-dashed border-slate-200 rounded-[2.5rem] text-center w-full max-w-xs bg-slate-50/50">
                        ...
                        <p className="text-[10px] text-slate-300 font-bold uppercase transition-spacing tracking-tighter">
                          BELUM DITETAPKAN
                        </p>
                      </div>
                    )}
                  ...
                  <div className="flex flex-wrap justify-center gap-4 w-full">
                    {(
                      selectedPanitiaForOrg.members.filter(
                        (m) =>
                          !["Ketua Panitia", "Setiausaha"].includes(m.role),
                      ) as PanitiaMember[]
                    ).map((m) => (
                      <OrgCard
                        key={m.id}
                    ...
                    {isAdmin &&
                      (["Ketua Panitia", "Setiausaha", "Guru Panitia"] as const)
                        .filter((role) => {
                          ...
                          return !selectedPanitiaForOrg.members.some(
                            (m) => m.role === role,
                          );
                        })
                        .map((role) => (
                          ...
                          >
                            ...
                            <p className="text-[8px] font-black text-slate-400 group-hover:text-blue-600 uppercase tracking-widest leading-tight">
                              Tambah {role}
                            </p>
                          </button>
                        ))}
              ...
              {allAvailableTeachers
                ...
                .map((t) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-black text-slate-800 leading-tight uppercase line-clamp-1">
                        {t.name}
                      </p>
            ...
            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 sm:p-8 bg-slate-950/95 backdrop-blur-2xl">
              ...
              >
                <button
                  ...
                  className="absolute top-0 right-0 sm:top-4 sm:right-4 z-50 w-12 h-12 bg-white/10 hover:bg-blue-600 hover:border-blue-600 border border-white/20 rounded-full flex items-center justify-center text-white transition-all backdrop-blur-md"
                >
                  <X className="w-6 h-6" />
                </button>
                ...
                <div
                  className="w-full h-full max-h-[85vh] flex items-center justify-center relative touch-pan-x cursor-pointer"
                  onTouchStart={(e) => {
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 bg-slate-900/60 font-mono text-xs font-bold text-white/70 rounded-full tracking-widest backdrop-blur-md border border-white/10">
                  ...
                  {kurikulumData.uasaPbdScreenshots.length}
                </div>
              </motion.div>
            </div>
          )}
    ...
    >
      ...
      >
        {member.photoUrl ? (
          ...
          <Users
            className={
              isSmall ? "w-8 h-8 text-slate-300" : "w-10 h-10 text-slate-300"
            }
          />
        )}
      </div>
      ...
      <h5
        className={`${isSmall ? "text-[11px]" : "text-sm"} font-black text-slate-800 leading-tight mb-1 uppercase tracking-tight line-clamp-2`}
      >
        {member.name}
      </h5>
      <div className="flex flex-col gap-1 items-center">
        <span
          ...
          style={{
            borderColor: `${panitiaColor}30`,
            color: panitiaColor,
            backgroundColor: `${panitiaColor}10`,
          }}
        >
          {member.role}
        </span>
