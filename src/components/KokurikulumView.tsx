...
import { 
  Trophy, Search, RefreshCw, AlertCircle, Database, Settings, 
  ChevronLeft, ChevronRight, Activity, Medal, Star, Award, Tent, Lightbulb, Sprout, Clipboard, Check
} from 'lucide-react';
...
export function KokurikulumView({ details, isAdmin, onSave }: KokurikulumViewProps) {
    ...
    try {
      ...
      try {
        ...
        if (text.trim().startsWith('<')) {
          throw new Error('Apps Script memulangkan HTML. Sila pastikan Apps Script anda di-deploy dengan betul.');
        }
        throw new Error('Data yang diterima bukan format JSON yang sah.');
      }

      const arrayData = Array.isArray(parsed) ? parsed : (parsed.data || parsed.records || parsed[activeTab] || []);
  ...
  const processedData = useMemo(() => {
    return loadedData.filter(item => {
      ...
      const tahunVal = String(getValueByKey(item, ['TAHUN', 'YEAR'])).trim().toUpperCase();
      const matchTahun = filterTahun === 'Semua' || tahunVal === filterTahun;

      const kelasVal = String(getValueByKey(item, ['KELAS', 'CLASS'])).trim().toUpperCase();
      const matchKelas = filterKelas === 'Semua' || kelasVal === filterKelas;

      const peringkatVal = String(getValueByKey(item, ['PERINGKAT', 'LEVEL'])).trim().toUpperCase();
      const matchPeringkat = filterPeringkat === 'Semua' || peringkatVal === filterPeringkat;

      const pencapaianVal = String(getValueByKey(item, ['PENCAPAIAN', 'ACHIEVEMENT'])).trim().toUpperCase();
      const matchPencapaian = filterPencapaian === 'Semua' || pencapaianVal === filterPencapaian;

      return matchSearch && matchTahun && matchKelas && matchPeringkat && matchPencapaian;
    });
  }, [loadedData, searchTerm, filterTahun, filterKelas, filterPeringkat, filterPencapaian]);
  ...
  const analytics = useMemo(() => {
    ...
    processedData.forEach(item => {
      const pencapaian = String(getValueByKey(item, ['PENCAPAIAN', 'ACHIEVEMENT'])).trim() || 'TIADA';
      const peringkat = String(getValueByKey(item, ['PERINGKAT', 'LEVEL'])).trim() || 'TIADA';

      pencapaianObj[pencapaian] = (pencapaianObj[pencapaian] || 0) + 1;
      peringkatObj[peringkat] = (peringkatObj[peringkat] || 0) + 1;
    });

    return { total, pencapaianCounts: pencapaianObj, peringkatCounts: peringkatObj };
  }, [processedData]);
  ...
  const getPeringkatColor = (p: string) => {
    ...
    if (upper.includes('ANTARABANGSA') || upper.includes('INTERNATIONAL')) return 'bg-purple-100 text-purple-700 border-purple-200';
    if (upper.includes('KEBANGSAAN') || upper.includes('NATIONAL')) return 'bg-amber-100 text-amber-700 border-amber-200';
    if (upper.includes('NEGERI') || upper.includes('STATE')) return 'bg-blue-100 text-blue-700 border-blue-200';
    if (upper.includes('DAERAH') || upper.includes('DISTRICT')) return 'bg-emerald-100 text-emerald-700 border-emerald-200';
    ...
    <div className="space-y-6 pb-12 w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between sm:items-center bg-white p-6 sm:p-8 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)]">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Pencapaian Kokurikulum</h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {isAdmin && (
            <button 
              onClick={() => { setTempUrl(details.kokoGasUrl || ''); setShowSettings(true); }}
      ...
      {errorMsg && (
        <div className="bg-red-50 border border-red-200 text-red-700 p-5 rounded-2xl flex items-start gap-3.5">
          ...
          <div>
            <p className="text-sm font-black uppercase tracking-wider mb-1">Ralat Semasa Integrasi</p>
      ...
      {!loading && !errorMsg && loadedData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="p-8 rounded-3xl border border-[#bc1437]/20 bg-[#bc1437] shadow-sm relative overflow-hidden flex flex-col justify-center items-center h-full">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-white opacity-90 mb-2">Jumlah Pencapaian</h4>
            <p className="text-6xl font-black text-white">{analytics.total}</p>
          </div>
          
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Pecahan Peringkat</h4>
            <div className="space-y-3">
              {Object.entries(analytics.peringkatCounts).sort((a,b) => (b[1] as number) - (a[1] as number)).slice(0, 4).map(([k, v], i) => (
                <div key={i}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="font-bold text-slate-700 truncate mr-2">{k}</span>
                    <span className="font-black text-[#bc1437]">{Number(v)}</span>
                  </div>
                  <div className="w-full bg-slate-100 h-1.5 rounded-full">
                    <div className="bg-[#bc1437] h-1.5 rounded-full" style={{ width: `${(Number(v)/(analytics.total || 1))*100}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Pencapaian Teratas</h4>
            <div className="space-y-3">
              {Object.entries(analytics.pencapaianCounts).sort((a,b) => (b[1] as number) - (a[1] as number)).slice(0, 4).map(([k, v], i) => (
                <div key={i}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="font-bold text-slate-700 truncate mr-2">{k}</span>
                    <span className="font-black text-amber-600">{Number(v)}</span>
                  </div>
                  <div className="w-full bg-slate-100 h-1.5 rounded-full">
                    <div className="bg-amber-400 h-1.5 rounded-full" style={{ width: `${(Number(v)/(analytics.total || 1))*100}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            >
              ...
              {availableTahun.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select
              value={filterKelas}
            ...
            >
              ...
              {availableKelas.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select
              value={filterPeringkat}
              onChange={e => { setFilterPeringkat(e.target.value); setCurrentPage(1); }}
              className="px-3.5 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl outline-none focus:border-[#bc1437] shadow-sm"
            >
              <option value="Semua">Semua Peringkat</option>
              {availablePeringkat.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select
              value={filterPencapaian}
              onChange={e => { setFilterPencapaian(e.target.value); setCurrentPage(1); }}
              className="px-3.5 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl outline-none focus:border-[#bc1437] shadow-sm"
            >
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-[#bc1437] text-white text-[10px] md:text-xs uppercase tracking-wider border-b border-[#bc1437]">
                ...
                <th className="px-6 py-4 font-black">Nama Murid</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={10} className="px-6 py-20 text-center text-slate-400">
                    ...
                    <p className="font-extrabold text-xs uppercase tracking-widest text-slate-500">Memuat naik data...</p>
                  </td>
                </tr>
                ...
                paginatedData.map((item, idx) => {
                  ...
                  const nama = String(getValueByKey(item, ['NAMA MURID', 'NAMA', 'NAME']));
                  const tahun = String(getValueByKey(item, ['TAHUN', 'YEAR']));
                  const kelas = String(getValueByKey(item, ['KELAS', 'CLASS']));
                  const pertandingan = String(getValueByKey(item, ['PERTANDINGAN', 'AKTIVITI', 'COMPETITION', 'ACTIVITY']));
                  const anjuran = String(getValueByKey(item, ['ANJURAN', 'ORGANIZER']));
                  const peringkat = String(getValueByKey(item, ['PERINGKAT', 'LEVEL']));
                  const pencapaian = String(getValueByKey(item, ['PENCAPAIAN', 'ACHIEVEMENT']));
                  const guru = String(getValueByKey(item, ['NAMA GURU PENGIRING', 'GURU PENGIRING', 'GURU', 'TEACHER']));

                  return (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors border-b border-slate-100">
                      <td className="px-6 py-4 text-center font-black text-slate-400 text-xs">{realIndex}</td>
                      <td className="px-6 py-4 font-bold text-slate-800 text-xs sm:text-sm uppercase">{nama || '-'}</td>
                      <td className="px-6 py-4 text-xs">
                        <div className="font-bold text-slate-700 uppercase">{tahun || '-'}</div>
                        <div className="font-medium text-slate-500 uppercase">{kelas || '-'}</div>
                      </td>
                      <td className="px-6 py-4 text-xs min-w-[200px] whitespace-normal">
                        <div className="font-bold text-slate-800 uppercase leading-snug">{pertandingan || '-'}</div>
                        <div className="font-medium text-slate-500 text-[10px] uppercase mt-1 leading-snug">{anjuran ? `Anjuran: ${anjuran}` : ''}</div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-block px-2.5 py-1 text-[10px] rounded-lg border font-bold uppercase tracking-wider ${getPeringkatColor(peringkat)}`}>
                          {peringkat || '-'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                         <span className="font-black text-amber-600 text-xs uppercase">{pencapaian || '-'}</span>
                      </td>
                      <td className="px-6 py-4 font-medium text-slate-600 text-xs uppercase line-clamp-2">{guru || '-'}</td>
                    </tr>
                  );
                })
              )}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-2xl w-full shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            <h4 className="font-black text-lg text-slate-800 mb-2">Tetapan Apps Script</h4>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-100 pb-4">URL API Kokurikulum (GET)</p>
            
            <label className="text-[10px] font-black uppercase text-slate-500 tracking-wider mb-2 block">URL Apps Script</label>
            <input 
              type="text" 
              value={tempUrl}
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Kod Apps Script (.gs)</label>
                <button 
                ...
                >
                  {copiedScript ? <><Check className="w-3 h-3 text-emerald-600" /> Disalin</> : <><Clipboard className="w-3 h-3" /> Salin Kod</>}
                </button>
              </div>
